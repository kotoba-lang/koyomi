(ns koyomi.cacao-test
  "Offline verification of agent-side CACAO minting. Server acceptance can't be
  tested here (a live kotobase.net run also needs the agent key authorized on
  the graph), but the crypto + encoding are fully checkable: canonical
  did:key, a verifying Ed25519 signature over the exact SIWE message, and a
  well-formed CBOR envelope. The per-actor key model is the koyomi/tayori/
  kekkai analog — the key IS the identity."
...
(deftest minted-cacao-signature-verifies
  (testing "the Ed25519 signature is over the exact SIWE message"
    (let [id    (c/generate-identity)
          grant {:cap :cap/read :scope "koyomi"}
...
(deftest minted-cacao-is-wellformed-cbor
  (let [id    (c/generate-identity)
        cacao (c/mint id {:cap :cap/transact :scope "koyomi"}
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/koyomi-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/koyomi"]
         (c/grant->resources {:cap :cap/read :scope "koyomi"})))
